...
export class ManageCouponsComponent {
  ...
  employees = computed(() => this.dataService.employees().filter(e => e.role === 'employee' && e.status === 'active'));
  ...
  handleGenerateCoupons() {
    ...
    if (this.generateCouponsForm.valid && this.selectedEmployee()) {
        ...
        const result = this.dataService.generateCouponsForEmployee(this.selectedEmployee()!.id, couponType!);
        if (result.success) {
  ...
  handleGenerateContractorCoupons() {
    ...
    if (this.generateContractorCouponsForm.valid && this.selectedContractor()) {
      ...
      const result = this.dataService.generateCouponsForContractor(this.selectedContractor()!.id, couponType!, quantity!);
       if (result.success) {
          this.statusMessage.set({ type: 'success', text: result.message });
          this.closeModals();
          setTimeout(() => this.statusMessage.set(null), 5000);
